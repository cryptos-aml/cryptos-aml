 ...
 * Mark a declaration as failed using its nonce (operator only)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate txHash if provided (optional for failed status)
    ...
    if (declaration.status === 'executed') {
      return NextResponse.json(
        { error: 'Cannot mark executed declaration as failed' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      declaration: {
        ...
        txHash: declaration.txHash || null,
      },
    });
  } catch (error) {
    console.error('Error marking declaration as failed:', error);
    return NextResponse.json(
